...
# -*- coding: utf-8 -*-
"""
PDF OCR 파싱을 위한 Python 스크립트
Java 백엔드에서 호출하여 PDF에서 보험기간, 납입기간, 가입나이 정보를 OCR로 추출
"""
...
class PDFOCRParser:
    ...
    def log(self, message: str):
        if self.debug:
            print(f"[PDFOCRParser] {message}", file=sys.stderr)
    ...
    def parse_insurance_terms_enhanced(self, block4: str) -> Dict[str, str]:
        """4.사업방법 섹션에서 보험기간, 납입기간, 가입나이 추출 (향상된 버전)"""
        terms = {
            'insuTerm': '',
            'payTerm': '',
            'ageRange': '',
    ...
    def parse_pdf_hybrid(self, pdf_path: str, insu_cd: str = None) -> Dict[str, any]:
        """하이브리드 PDF 파싱: 일반 텍스트 추출 + OCR"""
        try:
            # 1단계: 일반 PDF 텍스트 추출
            text_normal = self.extract_text_from_pdf(pdf_path)
            
            # 2단계: OCR 텍스트 추출
            text_ocr = self.extract_text_with_ocr(pdf_path)
            
            # 3단계: 두 결과를 결합하여 더 정확한 텍스트 생성
            combined_text = self.combine_texts(text_normal, text_ocr)
            
            # 섹션별 분리
            sections = self.find_sections(combined_text)
            
            # 보험기간, 납입기간, 가입나이 추출
            terms = self.parse_insurance_terms_enhanced(sections['block4'])
            ...
            self.log(f"하이브리드 PDF 파싱 오류: {e}")
            return {'error': str(e), 'success': False}
    
    def combine_texts(self, text_normal: str, text_ocr: str) -> str:
        """일반 텍스트와 OCR 텍스트를 결합하여 더 정확한 텍스트 생성"""
        # 간단한 결합 로직: OCR 텍스트가 더 길면 OCR 우선, 아니면 일반 텍스트 우선
        if len(text_ocr) > len(text_normal) * 1.5:
            return text_ocr
        else:
            return text_normal
...
def main():
    ...
    parser = PDFOCRParser()
    result = parser.parse_pdf_hybrid(pdf_path, insu_cd)
